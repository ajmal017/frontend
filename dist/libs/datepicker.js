...
Datepicker.prototype.init = function() {
    ...
    .on('_reposition.datepicker', function() {
        ...
        var swiper = $('.swiper-slide-active').outerHeight();
        self.$picker;
    })
    ...
    .on('click.datepicker', '.day:not(.invalid)', function(e) {
        var picked = moment($(this).find('div').attr('data-date'));
    ...
    .on('showdatepicker.datepicker', function(e) {
        ...
        self.$picker.hide().trigger('_render');
        $('.picker').css({'opacity':0,'transform':'scale(0)'}).show();
        setTimeout(function(){
            $('.picker').css({'opacity':1,'transform':'scale(1)'});
        },100);   
        lastopened = new Date();
...
Datepicker.prototype.month_table = function(displayed, value) {
    var self = this,
        ...
        t = ['<table><tr><th>', wd[1].charAt(0), '</th><th>', wd[2].charAt(0), '</th><th>', wd[3].charAt(0), '</th><th>', wd[4].charAt(0), '</th><th>', wd[5].charAt(0), '</th><th>', wd[6].charAt(0), '</th><th>', wd[0].charAt(0), '</th></tr>'];
    $.each(self.month_array(displayed), function(ix, week) {
        ...
        $.each(week, function(ix, day) {
            ...
            t.push("<td class='" + classes.join(' ') + "' title='" + title + "'><div data-date='" + repr + "'>" + label + "</div></td>");
        });
        t.push('</tr>');
    });
    t.push('</table>');
